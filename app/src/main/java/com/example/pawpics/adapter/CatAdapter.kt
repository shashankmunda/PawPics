...
class CatAdapter(val context: Context) : RecyclerView.Adapter<CatAdapter.CatView>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CatView {
        val view=LayoutInflater.from(parent.context).inflate(R.layout.cat_image_holder,parent,false)
        return CatView(view)
    }

    override fun onBindViewHolder(holder: CatView, position: Int) {
        holder.imageview.layoutParams.height= ((1.0f*(catsList[position].height)*(displayMetrics.widthPixels/2))/catsList[position].width).roundToInt()
